...
void Player::createVertices() {
    ...
    for(int i = 0; i < 18; i++){ //Per coprire la faccia superiore
        float x1 = 0.5*sin(glm::radians(i*10.0));
        float x2 = 0.5*sin(glm::radians((i+1)*10.0));
        float x3 = 0.5*sin(glm::radians(i*10.0+180));
        float x4 = 0.5*sin(glm::radians((i+1)*10.0+180));

        float z1 = 0.5*cos(glm::radians(i*10.0));
        float z2 = 0.5*cos(glm::radians((1+i)*10.0));
        float z3 = 0.5*cos(glm::radians(i*10.0+180));
        float z4 = 0.5*cos(glm::radians((1+i)*10.0+180));

        float y = 1.0f;

        vertices[648+i*18] = x1;
        vertices[648+i*18+1] = y;
        vertices[648+i*18+2] = z1;

        vertices[648+i*18+3] = x2;
        vertices[648+i*18+4] = y;
        vertices[648+i*18+5] = z2;

        vertices[648+i*18+6] = x3;
        vertices[648+i*18+7] = y;
        vertices[648+i*18+8] = z3;

        vertices[648+i*18+9] = x2;
        vertices[648+i*18+10] = y;
        vertices[648+i*18+11] = z2;

        vertices[648+i*18+12] = x4;
        vertices[648+i*18+13] = y;
        vertices[648+i*18+14] = z4;

        vertices[648+i*18+15] = x3;
        vertices[648+i*18+16] = y;
        vertices[648+i*18+17] = z3;
    }
}
...
void Player::calculateNormals() {
    ...
    for(int i = 0; i < 108; i++){
        normals[648+i*3] = 0.0;
        normals[648+i*3+1] = 1.0;
        normals[648+i*3+2] = 0.0;
    }
}
